...
NTL.phys.data.1999 <- filter(
  NTL.phys.data,
  year(sampledate) == 1999
)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday_90s <- filter(
  NTL.phys.data,
  lakename == "Tuesday Lake",
  year(sampledate) >= 1990,
  year(sampledate) <= 1999
)
...
> Answer: Because row numbers are arbitrary and may change, whereas filtering by variable values is reproducible and meaningful.
...
NTL.phys.data.Tuesday_July_90s <- 
  NTL.phys.data %>%
  filter(
    lakename == "Tuesday Lake",
    year(sampledate) >= 1990,
...
mean_surface_temp <- 
  NTL.phys.data.Tuesday_July_90s %>%
  filter(depth == 0) %>%
  summarize(mean_temp_C = mean(temperature_C, na.rm = TRUE))
